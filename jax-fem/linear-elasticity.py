...
def _open_with_viewer(vtk_path: str) -> None:
    ...
    vtk_path = "data/vtk/u.vtu"  # adjust if needed
    mesh = pv.read(vtk_path)

    print("Available fields:", mesh.array_names)

    # Check required fields
    if "sol" not in mesh.point_data:
        raise ValueError("Displacement field 'sol' not found in VTU file.")
    if "vm_stress" not in mesh.cell_data and "vm_stress" not in mesh.point_data:
        raise ValueError("Stress field 'vm_stress' not found in VTU file.")

    # Warp mesh by displacement
    scale = 1.0  # exaggeration factor; tune as needed
    mesh_def = mesh.warp_by_vector("sol", factor=scale)

    # Select stress data (cell or point)
    if "vm_stress" in mesh_def.cell_data:
        stress_data = mesh_def.cell_data["vm_stress"]
        stress_type = "cell"
    else:
        stress_data = mesh_def.point_data["vm_stress"]
        stress_type = "point"
